...
| `test_bootstrap_dry_run.py`, `windows_bootstrap_syscall_trace.ps1` | Native bootstrap plans; Windows scoped filesystem metadata/identity proof with an optional ETW witness; macOS persistent-object/network sandbox proof; syntax and approved source boundaries | PR-22 |
...
PR-32 deliberately retires the egress golden trio and gate tests. Current
payload tests assert absence of the gate and preserved native authority;
core instruction-migration tests cover known-original repair, custom conflicts,
rollback, and historical receipt readability. Independent credential, backup,
and filesystem suites remain in place.
