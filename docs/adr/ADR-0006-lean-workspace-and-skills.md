...
| ADR-0002 §3, §5, §7 | Seven-kind ceiling permits Skills, Library registration/cards, and minimal task-retention control metadata; routine per-event receipts become meaningful history; registered sources may reside in synced folders | Legible file state; rebuildable caches; goal verification; honest snapshots; one-way backup; no supported concurrent live sync of mutable Apparatus state |
