...
- `work_types` maps each work-type name to its managed Skill files.
...
existing `work_types: []` means no workflow filtering: init retains all
manifest-managed starter Skills while preserving the empty selection in
the profile.
...
It copies selected managed workflows, removes deselected managed workflows,
